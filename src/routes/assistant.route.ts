...
import { TrelloToolHandler, trelloToolHandlers } from '../services/trello';
...
function createToolEndpointHandler(action: string) {
  return async (req: Request, res: Response) => {
    try {
      const handler = (trelloToolHandlers as Record<string, TrelloToolHandler>)[action];
...
async function executeToolCalls(toolCalls: ToolCall[]): Promise<any[]> {
  ...
  for (const toolCall of toolCalls) {
    try {
      ...
      const handler = (trelloToolHandlers as Record<string, TrelloToolHandler>)[functionName];
      ...
      results.push({
        ...
        success: true,
        message:
          handlerResult?.message || `Action "${functionName}" exécutée avec succès`,
